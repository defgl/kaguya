...
$.isPanel = () =>
  ...
  $.lodash_get($input, "purpose") === "panel"
$.isTile = () =>
  $.isStash() &&
  typeof $script != "undefined" &&
  $.lodash_get($script, "type") === "tile"
...
!(async () => {
  ...
  const transformedTime = transformFont(
...
async function getDirectInfo() {
  ...
  try {
	...
	if (!res1.body.includes("中国") || res1.body.includes("香港") || res1.body.includes("澳门") || res1.body.includes("台湾")) {
		$.logErr("Check routing rules or current location, detected non-mainland IP")
	} else {
    ...
    // 翻译CN_ADDR
    let CN_ADDR_EN = (
      await Translator("DeepL", "zh", "en", CN_ADDR, {
        key: "7dda8ddf-e4c2-52a2-c350-09660439db14:fx",
      })
    )[0]
  ...
  try {
    ...
    CN_ADDR_EN = (
      await Translator("DeepL", "zh", "en", CN_ADDR, {
        key: "17bd2d86-a5df-9998-ff34-28075a83bc49:fx",
      })
    )[0]
    if (CN_IP && CN_ADDR) {
      return { CN_IP, CN_ADDR, CN_ADDR_EN }
...
async function Translator(
  ...
  target = "",
  text = "",
  api = { key: "17bd2d86-a5df-9998-ff34-28075a83bc49:fx" },
  database,
) {
  ...
  async function GetData(type, request) {
    ...
    await Fetch(request)
      ...
      .then((_data) => {
        switch (type) {
          case "DeepL":
            texts = _data?.translations?.map(
              (item) => item?.text ?? `翻译失败, 类型: ${type}`,
            )
            break
        }
      })
  ...
  if ($.isQuanX()) {
    switch (FORMAT) {
      ...
      case "application/grpc":
        // 返回二进制数据
        delete request.body
        if (ArrayBuffer.isView(request.bodyBytes))
...
async function getMovieInfo() {
	...
	try {
	  // 使用 Surge 的 HTTP 客户端发起请求
	  const res = await $httpClient.get('https://raw.githubusercontent.com/yes1am/douban-movie-calendar/master/douban-movie-calendar-2024.json');
	  const movies = JSON.parse(res.data); // 假设返回的数据是在 res.data 中
	  ...
	  // 在 Surge 中打印错误日志
	  console.error(`获取数据时发生错误: ${e.message || e}`);
	  // 或者使用 Surge 的日志方法（如果有的话）
	  //$log.error(`获取数据时发生错误: ${e.message || e}`);
	}
  
	// 在 Surge 中打印信息
	console.log(`${title} | ${rating}\n${comment}`);
	// 或者使用 Surge 的日志方法（如果有的话）
	//$log.info(`${title} | ${rating}\n${comment}`);
...
//   try {
//     const res = await $.http.get(
//       "https://raw.githubusercontent.com/yes1am/douban-movie-calendar/master/douban-movie-calendar-2024.json",
//     )
